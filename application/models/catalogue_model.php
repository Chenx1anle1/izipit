...
class Catalogue_model extends CI_Model {
	...
	public function __construct() {
  	...
  	public function catalogue() { //返回分类记录
		$query = $this->db->get_where('catalogue', array('cat_father' => "顶级"));
    ...
    public function have_son($name) {//返回父级分类记录
      $query = $this->db->get_where('catalogue', array('cat_father' => $name));
    ...
    public function name_by_another($another_name) {//返回别名分类记录
      $query = $this->db->get_where('catalogue', array('cat_another_name' => $another_name));
      foreach ($query->result_array() as $value) {
        return $value['cat_name'];
      }
    }

    public function is_father($another_name) {//返回父级分类记录
      $query = $this->db->get_where('catalogue', array('cat_another_name' => $another_name));
      foreach ($query->result_array() as $value) {
    ...
    public function add_type() {
      ...
      $db = $this->db->insert('catalogue',$catalogue);
      return $db;
    }
  	//UPDATE
  	//DELETE
