...
class Love_model extends CI_Model {
  ...
  public function is_love($uuid,$username) {
    $query = $this->db->get_where('love',array('love_pic' => $uuid,'love_user' => $username));
    return $query->num_rows();
  }

  public function pic_love($uuid) {
    $query = $this->db->get_where('love',array('love_pic' => $uuid));
    return $query->num_rows();
  }

  public function user_love($user,$page) {
    $query = $this->db->get_where('love',array('love_user' => $user),$this->pagenum,$this->pagenum*($page-1));
  ...
  public function add_love($uuid,$username) {
    ...
    $query = $this->db->insert('love',$love);
    return $query;
  }
	//UPDATE
	//DELETE
